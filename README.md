...
3. When invoking builds manually, run `dotnet build ./Penumbra.csproj --configuration Release` so you're using the same arguments as `scripts/bootstrap.sh`; local builds regenerate the README by default to keep documentation aligned with the compiled command metadata.
   - If you need to skip README generation temporarily (for example during quick smoke builds), set `RUN_GENERATE_README=false` in your environment **or** append `-p:RunGenerateREADME=false` to the build command—the flag is shared across both entry points so opt-outs behave consistently.
