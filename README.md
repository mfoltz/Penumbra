...
- **Changelog workflow:** Keep the `## Unreleased` section of `CHANGELOG.md` current. The `advance-version` workflow promotes those notes into a new `## <version>` heading during a release and restores an empty `## Unreleased` placeholder for future updates.
