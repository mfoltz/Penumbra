...
2. If you're working outside of `scripts/bootstrap.sh`, install the .NET 8 SDK locally so you mirror the `DOTNET_CHANNELS` defaults (`"8.0 6.0"`) used by the bootstrap script; keep the .NET 6 SDK alongside it so the net6.0 targeting pack stays available. Even though the project targets `net6.0`, it opts into preview C# features that require the newer compiler shipped with the .NET 8 SDK. Verify the installation with `dotnet --list-sdks`—you should see both `8.0.x` and `6.0.x` entries—or follow the [official SDK installation guide](https://learn.microsoft.com/dotnet/core/install/).
3. When invoking builds manually, run `dotnet build ./Penumbra.csproj --configuration Release` so you're using the same arguments as `scripts/bootstrap.sh`; both local runs and CI now regenerate the README automatically to keep documentation aligned with the compiled command metadata.
   - If you need to skip README generation temporarily (for example during quick smoke builds), set `RUN_GENERATE_README=false` in your environment **or** append `-p:RunGenerateREADME=false` to the build command—the shared MSBuild property applies across local and pipeline builds when you explicitly opt out.
   - Commit the regenerated README alongside related command updates so reviewers can anticipate the documentation diffs produced by the automation.
...
- **Changelog workflow:** Keep the `## WIP` section of [`CHANGELOG.md`](CHANGELOG.md) current. The version-advance workflows promote those notes into a new `## <version>` heading during a release and restore an empty `## WIP` placeholder for future updates. Draft your release notes under `## WIP` before triggering the bump in the next bullet.
- **Version bump workflows (`Advance version`, `Advance major`, `Advance minor`):** Use the GitHub Actions tab to run `Advance version` whenever you need to bump the version without touching code. Choose `major`, `minor`, or `patch` from the picker (or use the dedicated shortcut workflows to skip that choice), target the branch you want to update (defaults to `codex`), and provide a prerelease label such as `beta.1` only when you want the resulting version treated as a preview build. When a previous run stamped a prerelease (for example `1.3.0-beta.1`) and you omit the label on the follow-up run, the workflow now promotes that prerelease by reusing the existing `major.minor.patch` values—no extra increment required for the stable cut. The workflow updates `Penumbra.csproj`, `thunderstore.toml`, and [`CHANGELOG.md`](CHANGELOG.md) in-place, then either pushes directly (if the branch is unprotected) or opens a PR. It also emits the `new_version` output that downstream workflows—like guarded `Build` runs or automation consuming `workflow_call`—can use. Because the files are already bumped, the Build workflow’s version guard sees the new SemVer and allows CI to proceed on the next push.
- **Promoting a prerelease:** When the branch is ready for external testing, manually dispatch the `Build` workflow with `ready_to_ship=true`. That flips the workflow into prerelease mode: it updates `thunderstore.toml`, produces a Release build under `./bin/Release/net6.0/`, and uploads the DLL plus `CHANGELOG.md` to a GitHub prerelease tagged `v<version>-pre`. The tag matches the `new_version` output from the version-advance workflow—including any prerelease label—so the preview artifacts align with the version that triggered the build. Use this bundle when sanity-checking what will land on Thunderstore, then rerun the version bump without a label to promote the same numeric segments to the final release.
