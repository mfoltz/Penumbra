...
internal class Plugin : BasePlugin
{
    ...
    public class TokensConfig
    {
        ...
        public TokensConfig()
        {
            TokenSystem = InitConfigEntry("Config", "EnableCurrency", false, "Enable or disable token system.").Value;
            TokenItem = new(InitConfigEntry("Config", "CurrencyItemReward", 576389135, "Item prefab for currency reward.").Value);
            TokenRatio = InitConfigEntry("Config", "TokenRatio", 6, "Currency/reward factor.").Value;
            TokenRate = InitConfigEntry("Config", "CurrencyPerMinute", 5, "Tokens gained per minute online.").Value;
            DailyLogin = InitConfigEntry("Config", "DailyLogin", false, "Enable or disable daily login incentive.").Value;
            DailyItem = new(InitConfigEntry("Config", "DailyItem", -257494203, "Item prefab for daily reward.").Value);
            DailyQuantity = InitConfigEntry("Config", "DailyItemQuantity", 50, "Amount rewarded for daily login.").Value;
            UpdateInterval = InitConfigEntry("Config", "CurrencyUpdateInterval", 30, "Minutes between updates.").Value;
        }
    }
    ...
    */
    ...
    {
        // Config.Clear();
        // Remove only keys from sections that start with "Merchant"
