#!/usr/bin/env bash
set -e
BEPINEX_PLUGIN_DIR="/workspace/plugins"
...
dotnet build --no-restore -c Release -p:RunGenerateREADME=false "$PROJECT"

DLL_PATH="$SCRIPT_DIR/bin/Release/net6.0/Penumbra.dll"
